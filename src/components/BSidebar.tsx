...
const fallbackCountries = [
  { code: "bd", name: "Bangladesh", lat: 23.8103, lng: 90.4125, cities: [{ name: "Dhaka", lat: 23.8103, lng: 90.4125, address: "ID #9-N (New), 9-M(Old-N), 9th floor, Tower 1, Police Plaza Concord No.2, Road # 144, Gulshan Model Town, Dhaka 1215, Bangladesh", contacts: ["+880 1716 620989"] }] },
];
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (!hasInitialized) {
      const bangladesh = activeCountriesList.find(c => c.name === "Bangladesh");
      if (bangladesh && bangladesh.cities.length > 0) {
        const firstCountry = bangladesh;
        ...
        // Fallback to first country if Bangladesh not found
