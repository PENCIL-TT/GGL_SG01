...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const sortedCountries = [...activeCountriesList].sort((a, b) => a.name.localeCompare(b.name));
  ...
  useEffect(() => {
    if (!hasInitialized && sortedCountries.length > 0 && sortedCountries[0].cities.length > 0) {
  ...
  }, [sortedCountries, hasInitialized]);
