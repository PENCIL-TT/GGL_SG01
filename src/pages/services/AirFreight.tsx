...
import { Plane, Clock, Globe, Headset, CheckCircle } from "lucide-react";
...
const AirFreight = () => {
  ...
  React.useEffect(() => {
    fetch(`/api/service-details/${countryCode}/air-freight`)
      .then((res) => {
  ...
  const displayFeatures = data?.features_list && data.features_list.length > 0
    ? data.features_list.map((featTitle: string) => {
        ...
        return {
          title: featTitle,
          description: found?.description || "Professional air freight and cargo shipping solutions.",
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="md:w-1/2">
                ...
                >
                  {data?.hero_title || "Air Freight Solutions"}
                </motion.h1>
                <motion.p
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  {data?.hero_subtitle || "Tailored air freight solutions to meet your unique requirements"}
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <AspectRatio ratio={16 / 9}>
                    <img
                      ...
                      src={data?.hero_image_url || "/lovable-uploads/4fca88b2-3d5c-4588-809c-5d8429ca3bfe.png"}
                    />
                  </AspectRatio>
                </motion.div>
              </div>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto mb-12">
              <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center text-gray-800">
                {data?.section1_title || "Comprehensive Air Freight Services"}
              </h2>
              <div className="w-24 h-1 bg-brand-gold mx-auto mb-8"></div>
              <p className="text-gray-700 mb-6 font-normal text-justify whitespace-pre-wrap leading-relaxed">
            ...
            >
              <h3 className="text-2xl font-bold mb-4 text-slate-50">{data?.cta_title || "Ready to Ship with Us?"}</h3>
